...
impl ReplicationFetcher {
    // Adds the non existing incoming keys from the peer to be the fetcher. Returns the keys that has to be fetched
    ...
    pub(crate) fn add_keys(
        ...
        self.remove_keys(locally_stored_keys);
    ...
    // Returns the keys that has to be fetched from the peer/network.
    ...
    fn next_keys_to_fetch(&mut self) -> Vec<(RecordKey, Option<PeerId>)> {
        ...
        for (key, holders) in data_to_fetch {
            ...
            // If all holders / PEERS_TRIED_BEFORE_NETWORK_FETCH holders have exhausted MAX_FAILED_ATTEMPTS_PER_PEER,
    ...
    fn remove_keys(&mut self, existing_keys: &HashSet<RecordKey>) {
        self.to_be_fetched
