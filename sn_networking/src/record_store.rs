...
mod tests {
    ...
    fn gg() {
        ...
        let node_id_k = KBucketKey::from(node_id);
        ...
        let furthest = stored_record_keys
            ...
            .max_by_key(|k| {
                let kbucket_key = KBucketKey::from(k.to_vec());
                node_id_k_from_hex.distance(&kbucket_key)
            })
            .cloned()
            .unwrap();
        ...
        for failed in failed_puts {
            ...
            if incoming_record_key.distance(&node_id_k_from_hex)
                < furthest_record_key.distance(&node_id_k_from_hex)
            {
                println!("should have been pruned at least once");
            }
        }
    }
}
